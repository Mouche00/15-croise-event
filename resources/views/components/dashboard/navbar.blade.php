...
<div class="w-full flex items-start gap-2 z-[15] lg:z-[1] px-4 lg:p-0">
    <button onclick="showMenu()" type="button" class="lg:hidden">
        <svg class="w-8 h-8"
             viewBox="-16.91 0 178.11 178.11" xmlns="http://www.w3.org/2000/svg" fill="#000000">
    ...
    <div class="lg:h-20 w-full rounded-lg flex flex-col lg:flex-row justify-between items-center text-white border-4 border-dashed border-black bg-white lg:rotate-[-2deg]">

        @foreach(unserialize($links) as $link)
            @php
                $check = str_contains(url()->current(), $link);
                $color = $check ? 'magenta' : 'white';
                $inverse = $check ? 'white' : 'magenta';

                $text = $check ? 'white' : 'black';
                $inverseText = $check ? 'black' : 'white';
            @endphp

            <a href="{{ route($role . '.' . $link) }}" class="h-full w-full flex justify-center items-center px-8 py-2 text-{{ $text }} bg-{{ $color }} transition-all hover:bg-{{ $inverse }} hover:text-{{ $inverseText }}"> {{ ucwords($link) }} </a>
        @endforeach
    </div>
</div>
